...
| POST | `/payments/{orderId}/cancel` | 충전 취소 | - | - | ✅ USER | 🚧 미구현(선택). 결제 PENDING 중 타임아웃 전 사용자 직접 취소 |
...
| GET | `/yeopjeon/qr/status/{paymentRequestId}` | QR 결제 상태 폴링 | - | 결제 상태 정보 | ✅ USER | 🚧 미구현(필수). 푸시 미도달 시 사용자 확인 수단. STORY-13/14 작업 시 함께 구현 |
| POST | `/yeopjeon/qr/cancel` | QR 결제 취소(사용자) | paymentRequestId | - | ✅ USER | 🚧 미구현(선택). 현재 5분 자동 만료만 있음. STORY-15 연계 |
| POST | `/yeopjeon/qr/merchant/reissue` | QR 재발급 | - | 새 QR 정보 | ✅ MERCHANT | 🚧 미구현(선택). 분실·도용 의심 시 기존 QR 무효화 후 재발급. STORY-12 연계 |
...
> `description`(가게 소개)과 공지를 분리해 기간·만료·이력 관리를 가능하게 한다. `shop_notices` 테이블 신규 생성 필요. 가게 기본 관리(STORY-10) 완료 후 구현 권장.

| Method | Endpoint | 설명 | Request | Response | 인증 | 비고 |
| --- | --- | --- | --- | --- | --- | --- |
| POST | `/merchants/me/shop/notices` | 공지 등록 | title, content | 생성된 공지 | ✅ MERCHANT | 🚧 미구현 |
| GET | `/merchants/me/shop/notices` | 공지 목록 조회 | cursor, size | 공지 리스트 | ✅ MERCHANT | 🚧 미구현, Cursor 페이징 |
| DELETE | `/merchants/me/shop/notices/{noticeId}` | 공지 삭제 | - | - | ✅ MERCHANT | 🚧 미구현 |
