...
erDiagram
  ...
  payment_orders {
    bigint id PK
    varchar(36) order_uid UK "상인 주문번호(UUID), PG 전달용"
    bigint user_id FK
    bigint amount
    varchar(100) idempotency_key UK "중복 결제 방지"
    enum payment_method "CARD, KAKAO_PAY, NAVER_PAY, FOREIGN_CARD"
    enum status "PENDING, COMPLETED, FAILED, CANCELLED"
    varchar(100) pg_order_id "PG 요청 시 발급 ID"
    varchar(100) pg_transaction_id "PG 결제 완료 후 트랜잭션 ID"
    timestamp created_at
    timestamp updated_at
  }
