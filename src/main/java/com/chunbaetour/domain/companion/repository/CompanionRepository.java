...
public interface CompanionRepository extends JpaRepository<Companion, Long> {
    ...
    @Query("UPDATE Companion c SET c.status = com.chunbaetour.domain.companion.type.CompanionStatus.ENDED, "
            + "c.endedAt = CURRENT_TIMESTAMP, c.updatedAt = CURRENT_TIMESTAMP "
            + "WHERE c.status = com.chunbaetour.domain.companion.type.CompanionStatus.ONGOING AND c.tripEndDate < :today")
