...
public interface PlaceRepository extends JpaRepository<Place, Long> {
    ...
    /** 
     * 4-1. 인기 관광지 Fallback: ACTIVE 상태의 관광지를 (찜 * 0.7 + 조회수 * 0.3) 내림차순 정렬
     */
    @Query("SELECT p FROM Place p WHERE p.status = 'ACTIVE' ORDER BY (p.likeCount * 0.7 + p.viewCount * 0.3) DESC")
