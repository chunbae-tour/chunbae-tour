...
/**
 * 채팅 파일/이미지 업로드 서비스 (KAN-309, S-11c).
 * 파일 유효성 검사 + ChatFileStorage 위임 구조 — 운영=S3ChatFileStorage(@Profile prod), 그 외=LocalDiskChatFileStorage.
 * 반환값은 접근 URL이 아니라 <b>객체 키</b>다(조회/브로드캐스트 시 presigned GET으로 변환).
 *
 * [트랜잭션 경계]
 * uploadFile()은 DB 변경 없이 조회(멤버 확인) + 외부 저장소 업로드만 수행하므로 @Transactional 불필요.
 * 메시지(Message) 영속화는 별도 STOMP 전송(ChatMessageService.sendMessage)이 담당 — 업로드는 키만 반환한다
 * (고아 객체 문제는 키 미저장으로 회피, Shop 패턴 동일).
 *
 * [보안 경계]
...
public class ChatFileService {
    ...
    /**
     * 채팅 파일/이미지 업로드.
     * ACTIVE 멤버 확인 + CLOSED 방 차단 + 파일 유효성 검사(크기·content-type·magic-byte) 후
     * chatFileStorage에 위임하고 객체 키를 반환한다.
     * 파일명은 originalFilename을 쓰지 않고 ChatFileKeys가 UUID로 재생성한다(path traversal 차단).
     */
    public ChatFileUploadResponse uploadFile(Long userId, Long chatRoomId, MultipartFile file) {
        ...
        // 종료된 방은 업로드 차단 — 업로드만 되고 메시지 전송이 막혀 S3 고아 객체가 확정 발생하는 것을 사전 차단(Shop 패턴 동일).
        ...
        String objectKey = chatFileStorage.upload(chatRoomId, file);
        return new ChatFileUploadResponse(objectKey, sanitizeFileName(file.getOriginalFilename()), file.getSize(), file.getContentType());
    }

    private void validateFile(MultipartFile file) {
        // Spring @RequestParam 누락 시 서비스 도달 전 400 반환 — null은 실제 발생하지 않음
        if (file.isEmpty()) {
            throw new BusinessException(ErrorCode.CHAT_FILE_EMPTY);
        }
        ...
        if (IMAGE_CONTENT_TYPES.contains(contentType)) {
            if (file.getSize() > MAX_IMAGE_SIZE) {
                throw new BusinessException(ErrorCode.CHAT_IMAGE_FILE_TOO_LARGE);
            }
     ...
     * "허용 포맷 중 하나"가 아니라 "선언값과 일치"를 봐야 declared=png/실제=jpeg 같은 불일치를 막는다(Shop 패턴 동일).
     */
