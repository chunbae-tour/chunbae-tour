...
class ChatRedisPubSubServiceTest {
    ...
    private ChatRedisPubSubService service;

    private static final Long ROOM_ID = 123L;
    private static final String ACTUAL_CHANNEL = "chat:" + ROOM_ID;
    ...
    @BeforeEach
    void setUp() {
        service = new ChatRedisPubSubService(
                stringRedisTemplate, objectMapper, messagingTemplate, meterRegistry);
    }

    // onMessage() — PatternTopic 경유 시 실제 채널("chat:123")을 message.getChannel()로 추출해 브로드캐스트
    // 핵심 회귀 가드: MessageListenerAdapter가 패턴("chat:*")을 channel로 넘기던 버그 수정 검증
    @Test
    void onMessage_extractsActualChannelFromMessage_notPattern() throws Exception {
        ...
        given(objectMapper.readValue(eq("{}"), eq(ChatMessageResponse.class))).willReturn(RESPONSE);

        service.onMessage(message, PATTERN.getBytes(StandardCharsets.UTF_8));

        // "/sub/chat/rooms/123" — 리터럴 패턴("*") 아닌 실제 방ID로 발행되어야 함
        verify(messagingTemplate).convertAndSend(eq("/sub/chat/rooms/" + ROOM_ID), eq(RESPONSE));
    }

    // publish() — chat:{roomId} 채널로 직렬화된 JSON 발행 검증
    @Test
    void publish_sendsSerializedJsonToRedisChannel() throws Exception {
        String json = "{\"chatRoomId\":123}";
    ...
    void handleMessage_validInput_broadcastsToStompTopic() throws Exception {
        String json = "{}";
        given(objectMapper.readValue(eq(json), eq(ChatMessageResponse.class))).willReturn(RESPONSE);
    ...
    void handleMessage_wrongChannelPrefix_skipsProcessing() throws Exception {
        service.handleMessage("{}", "support:123");

        verify(objectMapper, never()).readValue(anyString(), eq(ChatMessageResponse.class));
    ...
    void handleMessage_invalidJson_doesNotThrow() throws Exception {
        given(objectMapper.readValue(anyString(), eq(ChatMessageResponse.class)))
    ...
    // publish() — Redis 발행 실패 시 예외 미전파 + chat.publish.failure.total 카운터 증가
    @Test
    void publish_redisFailure_doesNotThrow_andIncrementsMetric() throws Exception {
        given(meterRegistry.counter(ChatRedisPubSubService.METRIC_PUBLISH_FAILURE)).willReturn(counter);
    ...
    // handleMessage() — roomId 없는 채널("chat:") → 전송 미호출
    @Test
    void handleMessage_blankRoomId_skipsProcessing() throws Exception {
        given(objectMapper.readValue(anyString(), eq(ChatMessageResponse.class))).willReturn(RESPONSE);

        service.handleMessage("{}", "chat:");
    ...
    void handleMessage_broadcastFailure_doesNotThrow_andIncrementsMetric() throws Exception {
        given(objectMapper.readValue(anyString(), eq(ChatMessageResponse.class))).willReturn(RESPONSE);
        given(meterRegistry.counter(ChatRedisPubSubService.METRIC_BROADCAST_FAILURE)).willReturn(counter);
